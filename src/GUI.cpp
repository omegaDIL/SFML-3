...
std::unique_ptr<sf::Font> Interface::InterfaceText::m_font{};
std::vector<Interface*> Interface::m_everyMenu{};
size_t MenuInterface::m_pointingButton{ 0 };
std::unique_ptr<sf::Sprite> MenuInterface::m_backgroundPointingButton{};
...
{
	if (Interface::InterfaceText::m_font)
		return std::nullopt;
	...
	try
	{
		...
		Interface::InterfaceText::m_font = std::make_unique<sf::Font>(path);
		//if (!m_font->openFromFile(path))
		//	throw LoadingUIRessourceException{ "Failed to load fon at: " + path };

		(Interface::InterfaceText::m_font)->setSmooth(true);
	}
...
Interface::Interface(sf::RenderWindow* window, std::string backgroundPath)
	: m_window{ window }, m_spriteBackground{ nullptr }, m_actualTextureBackground{}, m_textureBackground{}
{
	Interface::InterfaceText::loadFont();
...
{
	// This buffer contains the texture of the background.
	sf::RenderTexture bufferForTexture{ sf::Vector2u{ windowSize.size.x, windowSize.size.y } };

	// Contains the default color of the background in an image.
	sf::Image imgDefaultBackground{ sf::Vector2u{ windowSize.size.x, windowSize.size.y }, sf::Color{20, 20, 20} };
	
	sf::Texture textureDefaultBackground{ imgDefaultBackground };
	...
	background.setPosition(sf::Vector2f{ windowSize.size.x / 2.f, windowSize.size.y / 2.f });
	background.setOrigin(sf::Vector2f{ m_actualTextureBackground.getSize().x / 2.f, m_actualTextureBackground.getSize().y / 2.f });
	...
	m_spriteBackground = std::make_unique<sf::Sprite>(m_textureBackground);
	m_spriteBackground->setPosition(sf::Vector2f{ windowSize.size.x / 2.f, windowSize.size.y / 2.f });
	m_spriteBackground->setOrigin(sf::Vector2f{ windowSize.size.x / 2.f, windowSize.size.y / 2.f });
}
